 ...
 * const authEntry = simResult.result!.auth[0]; // the smart-account entry
 *
 ...
 *
 * // 3a. (Optional) Let the kit auto-suggest rule IDs from on-chain rules
 ...
 *
 * // 3b. (Optional) Get just the IDs if you trust the suggestions
 * const ruleIds = await kit.resolveContextRuleIds(authEntry);
 * // → [1, 0]
 *
 * // 4. Pass to the signing operation
 * await kit.signAndSubmit(assembledTx, { contextRuleIds: ruleIds });
 * // or
 * await kit.multiSigners.operation(assembledTx, signers, { contextRuleIds: [1, 0] });
 * ```
 */
 ...
 * The list index matches `context_rule_ids` position.
 *
 * @example
 * ```typescript
 * const nodes = walkInvocationTree(authEntry.rootInvocation());
...
function walkRecursive(
  ...
  if (fn.switch().name === "sorobanAuthorizedFunctionTypeContractFn") {
    ...
    node.contractAddress = Address.fromScAddress(contractFn.contractAddress()).toString();
 ...
 *
 * @throws Error if lengths do not match
 */
 ...
 * 2. `CreateContract(wasmHash)` — matches when the node is a contract creation with that hash
  ...
  return nodes.map((node) => {
    ...
    for (const rule of rules) {
      ...
      if (ct.tag === "CallContract") {
        ...
        // Only relevant for wasm-upload invocations; contract address is not set
    ...
    // Sort by specificity: CallContract → CreateContract → Default
 ...
 *
 * This is the non-interactive version of `hintContextRuleIds` — it returns
